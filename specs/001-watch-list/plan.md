# Implementation Plan: 視聴予定リスト

**Branch**: `001-watch-list` | **Date**: 2025-11-20 | **Spec**: [specs/001-watch-list/spec.md](./spec.md)
**Input**: Feature specification from `/specs/001-watch-list/spec.md`

**Note**: This template is filled in by the `/speckit.plan` command. See `.specify/templates/commands/plan.md` for the execution workflow.

## Summary

ドラマ・アニメ・映画を「視聴予定」として登録し、タイトル→シリーズ→エピソードの階層構造で管理する機能を実装します。内部的にはすべてのタイトルが最低1つのシリーズ・エピソードを持つ統一的なデータモデルを採用し、UI上では階層の深さに応じてレイヤーを動的に非表示にすることでUXを最適化します。視聴状態の管理（未視聴/視聴済み）、複数回視聴履歴の記録、タイトル・状態によるフィルタリングをサポートします。技術スタックは、バックエンドにSpring Boot + MyBatis + PostgreSQL、フロントエンドにNext.js + React + TypeScriptを採用し、DDD + オニオンアーキテクチャとTDD原則に従って実装します。

## Technical Context

**Language/Version**:
- Backend: Java 17 (LTS)
- Frontend: TypeScript 5.3

**Primary Dependencies**:
- Backend: Spring Boot, MyBatis, PostgreSQL, Flyway
- Frontend: Next.js, React
- Testing: JUnit (Backend), Jest + Testing Library (Frontend), Playwright (E2E)

**Storage**: PostgreSQL
- スキーマ設計: 正規化（第3正規形）+ JSONB（URL リスト） + 外部キー制約 + CASCADE DELETE
- マイグレーション: Flyway（SQL ベース、バージョン管理）
- 詳細: `data-model.md` を参照

**Testing**:
- Backend: JUnit + Spring Test
- Frontend: Jest + React Testing Library
- E2E: Playwright（Next.js 公式推奨、クロスブラウザ対応）

**Target Platform**: Web Application (Linux server for backend, browser for frontend)

**Project Type**: Web (backend + frontend)

**Performance Goals**:
- 検索・フィルタ結果を1秒以内に表示（p95 < 1秒）
- API レスポンスタイム: p95 < 200ms
- データベースクエリ: p95 < 50ms
- 最小限500タイトル以上のデータを安定管理
- 同時接続ユーザー数: 初期 10 ユーザー、拡張 100+ ユーザー
- リクエスト/秒: 100 req/s

**Constraints**:
- 複数階層構造（タイトル→シリーズ→エピソード）の動的UI切り替え
- 視聴履歴の複数回記録とパフォーマンス最適化
- メモリ制約（開発環境）: Backend 512MB, Frontend 256MB, DB 256MB
- レスポンスタイム: API p95 < 200ms, ページ p95 < 1秒

**Scale/Scope**:
- 初期目標: 500 タイトル以上
- 拡張目標: 1000+ タイトル以上
- 1タイトルあたり平均 2-3 シリーズ（最大 20）、1シリーズあたり平均 12 エピソード（最大 100）
- 1エピソードあたり平均 1-2 回視聴（最大 10）、平均 2-3 件 URL（最大 10）
- 想定ユーザー数: 初期 1-10 ユーザー、拡張 100+ ユーザー
- データボリューム上限: 1000 タイトル × 3 シリーズ × 12 エピソード = 36,000 エピソードでパフォーマンステスト

## Constitution Check

*GATE: Must pass before Phase 0 research. Re-check after Phase 1 design.*

### I. DDD + オニオンアーキテクチャ準拠
- ✅ **Domain層の純粋性**: Title, Series, Episode, ViewingRecord をドメインエンティティとして定義し、フレームワーク非依存の純粋Javaで実装
- ✅ **依存性逆転**: domain/repository にインターフェース、infrastructure/persistence に MyBatis 実装を配置
- ✅ **層分離**: Presentation (REST Controller) → Application (UseCase) → Domain → Infrastructure の依存方向を遵守

### II. 仕様主導開発準拠
- ✅ **仕様先行**: spec.md → plan.md → tasks.md → 実装 の順序を遵守
- ✅ **API契約**: OpenAPI スキーマを Phase 1 で作成（contracts/ 配下）
- ✅ **上流更新**: 仕様変更時は spec.md → 契約 → 実装 の順に整合性を維持

### III. テスト重視とリグレッション防止準拠
- ✅ **テスト戦略明確化**: 単体テスト（Domain/Application層）、統合テスト（レイヤ間連携）、E2E テスト（ユーザーストーリー全体）を定義
- ✅ **カバレッジ目標**: ドメイン層・アプリケーション層で80%以上のコードカバレッジを目標
- ⚠️ **TDD実践**: Phase 2 で Red-Green-Refactor サイクルを実施（具体的な手順は tasks.md で定義）

### IV. 保守性とシンプルさの優先準拠
- ✅ **過度な抽象化を避ける**: 必要最小限のドメインモデル（4エンティティ）で実装、無駄な抽象層は作成しない
- ✅ **DDD の複雑性は許容**: 長期保守性のためのオニオンアーキテクチャ採用は妥当

### V. テスト駆動開発（TDD）採用準拠
- ✅ **TDD プラクティス**: Phase 2 実装時に Red-Green-Refactor サイクルを適用
- ⚠️ **テストファースト原則**: 機能実装前にテストを記述（tasks.md で具体化）

### VI. ドキュメント言語の統一と英語コーディング準拠
- ✅ **ドキュメント日本語**: spec.md, plan.md, tasks.md は日本語で記述
- ✅ **コード英語**: クラス名・メソッド名・変数名は英語で記述
- ✅ **ユビキタス言語定義**: data-model.md で日本語↔英語の用語対応表を定義
  - 「タイトル」= "Title"、「シリーズ」= "Series"、「エピソード」= "Episode"、「視聴履歴」= "ViewingRecord"
  - 「視聴状態」= "WatchStatus"、「タイトル情報URL」= "TitleInfoUrl"、「視聴ページURL」= "WatchPageUrl"

### アーキテクチャ方針準拠
- ✅ **バックエンドレイヤ構造**: domain, application, infrastructure, presentation の4層構造を採用
- ✅ **フロントエンド設計**: Next.js App Router + src/app/, src/components/, src/lib/ の構造を採用
- ✅ **MyBatis 選択**: SQL 明示的制御、ドメイン層独立性、リポジトリ実装柔軟性を活用

### 技術スタック準拠
- ✅ **Backend**: Java + Spring Boot + MyBatis + PostgreSQL + Flyway + JUnit
- ✅ **Frontend**: TypeScript + React + Next.js + Jest + Testing Library
- ✅ **API 契約**: REST API + OpenAPI

### インフラ・実行環境準拠
- ✅ **docker compose 必須**: 開発環境で docker compose による起動・管理を実施
- ✅ **設定管理**: .env ファイル + compose 環境変数で管理、コードへのベタ書き禁止
- ✅ **リポジトリ構成**: backend/, frontend/, docker-compose.yml, .specify/ の構成を遵守

**GATE 判定 (Phase 1 完了後)**:
- ✅ PASS
- Phase 0 で NEEDS CLARIFICATION 項目をすべて解決（research.md）
- Phase 1 で data-model.md, contracts/openapi.yaml, quickstart.md を生成
- ユビキタス言語定義完了（data-model.md）
- 残り ⚠️ 項目（TDD 実践、テストファースト原則）は Phase 2（tasks.md + 実装）で解決

## Project Structure

### Documentation (this feature)

```text
specs/[###-feature]/
├── plan.md              # This file (/speckit.plan command output)
├── research.md          # Phase 0 output (/speckit.plan command)
├── data-model.md        # Phase 1 output (/speckit.plan command)
├── quickstart.md        # Phase 1 output (/speckit.plan command)
├── contracts/           # Phase 1 output (/speckit.plan command)
└── tasks.md             # Phase 2 output (/speckit.tasks command - NOT created by /speckit.plan)
```

### Source Code (repository root)

```text
backend/
├── src/
│   ├── main/
│   │   ├── java/com/example/videowatchlog/
│   │   │   ├── domain/
│   │   │   │   ├── model/          # Title, Series, Episode, ViewingRecord エンティティ
│   │   │   │   ├── repository/     # リポジトリインターフェース（依存性逆転）
│   │   │   │   └── service/        # ドメインサービス
│   │   │   ├── application/
│   │   │   │   ├── service/        # ユースケース実装
│   │   │   │   ├── dto/            # データ転送オブジェクト
│   │   │   │   └── usecase/        # ユースケースクラス
│   │   │   ├── infrastructure/
│   │   │   │   ├── persistence/    # MyBatis Mapper 実装
│   │   │   │   └── config/         # Spring Boot 設定
│   │   │   └── presentation/
│   │   │       └── controller/     # REST API コントローラ
│   │   └── resources/
│   │       ├── db/migration/       # Flyway マイグレーションスクリプト
│   │       └── application.yml     # アプリケーション設定
│   └── test/
│       └── java/com/example/videowatchlog/
│           ├── domain/             # ドメイン層単体テスト
│           ├── application/        # アプリケーション層統合テスト
│           └── integration/        # レイヤ間統合テスト
└── pom.xml

frontend/
├── src/
│   ├── app/                        # Next.js App Router ページ
│   │   ├── page.tsx               # トップページ（タイトル一覧）
│   │   ├── titles/                # タイトル関連ページ
│   │   │   ├── [id]/             # タイトル詳細ページ
│   │   │   └── new/              # タイトル作成ページ
│   │   └── layout.tsx            # 共通レイアウト
│   ├── components/                # 再利用可能なコンポーネント
│   │   ├── title/                # タイトル関連コンポーネント
│   │   ├── series/               # シリーズ関連コンポーネント
│   │   ├── episode/              # エピソード関連コンポーネント
│   │   └── common/               # 共通コンポーネント
│   ├── lib/                       # ユーティリティ・API クライアント
│   │   ├── api/                  # API クライアント
│   │   └── utils/                # ヘルパー関数
│   └── types/                     # TypeScript 型定義
└── tests/
    ├── unit/                      # コンポーネント単体テスト
    └── integration/               # API 連携統合テスト

docker-compose.yml                 # 開発環境 docker compose 定義
.env.example                       # 環境変数テンプレート
.specify/                          # Spec Kit（仕様・計画・タスク）
```

**Structure Decision**: Web アプリケーション構成（Option 2）を選択。バックエンドは DDD + オニオンアーキテクチャに準拠した4層構造（domain, application, infrastructure, presentation）で実装し、フロントエンドは Next.js App Router を使用した src/app/ + src/components/ + src/lib/ の構造を採用します。docker compose により、開発環境でバックエンド・フロントエンド・PostgreSQL を統合管理します。

## Complexity Tracking

> **Fill ONLY if Constitution Check has violations that must be justified**

該当なし。Constitution Check は全項目でPASSしています。

---

## Phase Completion Summary

### Phase 0: Outline & Research ✅ COMPLETED

**成果物**: `research.md`

**解決した NEEDS CLARIFICATION 項目**:
1. Java バージョン → **Java 17 (LTS)**
2. TypeScript バージョン → **TypeScript 5.3**
3. DB スキーマ設計 → **正規化 + JSONB + 外部キー制約 + CASCADE DELETE**
4. マイグレーション戦略 → **Flyway（SQL ベース）**
5. E2E テストツール → **Playwright**
6. パフォーマンス目標 → **同時接続 10 ユーザー（初期）、100 req/s、p95 < 200ms（API）、p95 < 1秒（ページ）**
7. メモリ制約 → **Backend 512MB, Frontend 256MB, DB 256MB（開発環境）**
8. データボリューム → **初期 500 タイトル、拡張 1000+ タイトル、36,000 エピソードでパフォーマンステスト**

**技術選定**:
- DDD ドメインモデルのベストプラクティス調査完了
- MyBatis + PostgreSQL 統合のベストプラクティス調査完了
- Next.js + React のベストプラクティス調査完了

### Phase 1: Design & Contracts ✅ COMPLETED

**成果物**:
1. `data-model.md` - ドメインエンティティ、値オブジェクト、リレーション、バリデーションルール、DB スキーマ
2. `contracts/openapi.yaml` - REST API 契約（OpenAPI 3.0.3）
3. `quickstart.md` - 開発環境セットアップガイド

**データモデル設計**:
- エンティティ: Title, Series, Episode, ViewingRecord
- 値オブジェクト: TitleInfoUrl, WatchPageUrl
- 列挙型: WatchStatus (UNWATCHED, WATCHED)
- ユビキタス言語（日本語↔英語）対応表完成
- DB スキーマ（PostgreSQL）定義完成

**API 契約設計**:
- タイトル管理: GET/POST/PUT/DELETE /titles
- シリーズ管理: POST/GET/PUT/DELETE /series
- エピソード管理: POST/GET/PUT/DELETE /episodes
- 視聴履歴管理: POST/DELETE /viewing-records
- 視聴完了: POST /episodes/{id}/complete

**Agent Context 更新**:
- `.specify/scripts/bash/update-agent-context.sh claude` 実行完了
- CLAUDE.md に本機能の技術スタック・構成を追加

### Phase 2: Tasks & Implementation 🔜 NEXT

**次のステップ**:
1. `/speckit.tasks` コマンドを実行し、`tasks.md` を生成
2. TDD（Red-Green-Refactor サイクル）に従って実装タスクを定義
3. 実装順序: ドメイン層 → アプリケーション層 → インフラ層 → プレゼンテーション層
4. 各タスクにテストファーストでテストケースを記述

---

### Phase 3: Architecture Improvement - CQRS + Independent Aggregates 🔜 NEXT

**背景**: Phase 1-2 の実装により、Episode + ViewingRecord の集約化が完了したが、Title → Series → Episode → ViewingRecord の Giant Aggregate パターンがN+1クエリ問題を引き起こしている（GetAllTitles: 50タイトル時に101クエリ）。

**目標**:
1. Series と Episode を独立した集約に昇格（DDD ベストプラクティス準拠）
2. CQRS パターンを適用し、Read Model で JOIN を使用して N+1 クエリを完全解決（99% 削減）
3. Write Model（ドメインモデル）とRead Model（クエリ最適化）を明確に分離
4. 全140テストが通過し、パフォーマンス改善を確認

**技術アプローチ**:
- **Write Model**: Title、Series、Episode を独立した集約として管理（ID参照のみ）
- **Read Model**: TitleReadModel、SeriesReadModel、EpisodeReadModel で JOIN 最適化
- **Repository パターン**: Title、Series、Episode ごとに独立した Repository
- **MyBatis**: Write用と Read用で別々の Mapper を管理

#### 3.1 集約境界の再設計

**案A: 3つの独立集約（推奨）** vs **案B: 1つの大きな集約**

**案A採用の理由**:
- DDDベストプラクティス準拠（Eric Evans、Vaughn Vernon が推奨）
- Series と Episode を独立した集約に昇格
- スケーラビリティが高い（Series/Episode が増えても Title のロード時間は一定）
- 責任の分離が明確（各Repository が独立した責任）
- テストが簡潔（各集約を独立してテスト可能）
- CQRS パターンで JOIN を使用すれば N+1 クエリ問題も完全解決可能

**新しい集約構造**:
```
Title (集約ルート 1)
├── id: Long
├── name: String
└── titleInfoUrls: Set<TitleInfoUrl>

Series (集約ルート 2) ← 独立集約
├── id: Long
├── titleId: Long ← ID参照のみ
└── name: String

Episode (集約ルート 3) ← 独立集約
├── id: Long
├── seriesId: Long ← ID参照のみ
├── episodeInfo: String
├── watchPageUrls: List<WatchPageUrl>
└── viewingRecords: List<ViewingRecord> ← Episode 集約の一部
```

#### 3.2 CQRS パターンの適用

**Write Model（書き込み用）**:
- Title, Series, Episode が独立集約（オブジェクト参照なし）
- CreateTitle, CreateSeries, CreateEpisode などの書き込み操作に使用
- TitleRepository, SeriesRepository, EpisodeRepository
- ドメインロジック（バリデーション、ビジネスルール）を保持

**Read Model（読み取り専用）**:
- GetAllTitles, GetTitleDetail などの読み取り操作専用
- JOINを使って効率的にデータを取得（1クエリ）
- TitleListReadModel, TitleDetailReadModel
- 専用のMapper: TitleReadMapper（MyBatis）
- DTOに近い構造（ビジネスロジックなし）

**リードモデルの設計**:

TitleListReadModel（タイトル一覧用）:
```java
public class TitleListReadModel {
    private Long id;
    private String name;
    private LocalDateTime createdAt;
    private LocalDateTime updatedAt;
    // Getter のみ（イミュータブル）
}
```

TitleDetailReadModel（タイトル詳細用、Seriesとっ Episodeを包含）:
```java
public class TitleDetailReadModel {
    private Long id;
    private String name;
    private List<String> titleInfoUrls;
    private List<SeriesReadModel> series;
    private LocalDateTime createdAt;
    private LocalDateTime updatedAt;
}

public class SeriesReadModel {
    private Long id;
    private String name;
    private List<EpisodeReadModel> episodes;
    private LocalDateTime createdAt;
    private LocalDateTime updatedAt;
}

public class EpisodeReadModel {
    private Long id;
    private String episodeInfo;
    private String watchStatus;
    private List<String> watchPageUrls;
    private int viewingRecordCount;
    private LocalDateTime createdAt;
    private LocalDateTime updatedAt;
}
```

**MyBatis SQL（TitleReadMapper.xml）**:
```xml
<!-- タイトル一覧取得（シンプル） -->
<select id="findAllForList" resultType="TitleListReadModel">
    SELECT id, name, created_at AS createdAt, updated_at AS updatedAt
    FROM titles
    ORDER BY created_at DESC
</select>

<!-- タイトル詳細取得（JOIN で全て取得） -->
<select id="findByIdWithSeriesAndEpisodes" resultMap="TitleDetailResultMap">
    SELECT
        t.id AS title_id, t.name AS title_name,
        s.id AS series_id, s.name AS series_name,
        e.id AS episode_id, e.episode_info, e.watch_status,
        tiu.url AS title_info_url,
        (SELECT COUNT(*) FROM viewing_records WHERE episode_id = e.id) AS viewing_record_count
    FROM titles t
    LEFT JOIN title_info_urls tiu ON t.id = tiu.title_id
    LEFT JOIN series s ON t.id = s.title_id
    LEFT JOIN episodes e ON s.id = e.series_id
    WHERE t.id = #{id}
    ORDER BY s.created_at, e.created_at
</select>
```

**TitleReadService**:
```java
@Service
public class TitleReadService {
    private final TitleReadMapper titleReadMapper;

    public List<TitleListReadModel> getAllTitles() {
        return titleReadMapper.findAllForList();  // 1 クエリ
    }

    public Optional<TitleDetailReadModel> getTitleDetail(Long titleId) {
        return titleReadMapper.findByIdWithSeriesAndEpisodes(titleId);  // 1 クエリ（JOIN）
    }
}
```

**実装 Milestone**:
1. **Milestone 1** - Write Model の分離（Phase 1）: Title、Series、Episode のオブジェクト参照を削除、ID 参照に置き換え
2. **Milestone 2** - Read Model の追加（Phase 2）: CQRS パターン適用、JOIN 最適化実装
3. **Milestone 3** - UseCase 修正（Phase 3）: GetAllTitles、GetTitleDetail を Read Model に切り替え、CreateSeries/CreateEpisodeUseCase を簡潔化
4. **Milestone 4** - 全テスト検証（Phase 4）: 140テスト通過、パフォーマンス確認

**期待される効果**:
- GetAllTitles: 101 クエリ → 1 クエリ（99% 削減）
- GetTitleDetail: 2-3 クエリ → 1 クエリ（50-66% 削減）
- DDD ベストプラクティス完全準拠
- スケーラビリティ向上（Series/Episode 数増加の影響最小化）
- データ整合性の向上（CreateSeriesUseCase と CreateEpisodeUseCase の同期問題解決）

#### 3.3 実装ファイル一覧

**新規作成（Read Model）**:
- `backend/src/main/java/com/example/videowatchlog/application/readmodel/TitleListReadModel.java`
- `backend/src/main/java/com/example/videowatchlog/application/readmodel/TitleDetailReadModel.java`
- `backend/src/main/java/com/example/videowatchlog/application/readmodel/SeriesReadModel.java`
- `backend/src/main/java/com/example/videowatchlog/application/readmodel/EpisodeReadModel.java`
- `backend/src/main/java/com/example/videowatchlog/application/readmodel/mapper/TitleReadMapper.java`
- `backend/src/main/java/com/example/videowatchlog/application/readmodel/service/TitleReadService.java`
- `backend/src/main/resources/mybatis/mapper/readmodel/TitleReadMapper.xml`

**修正対象（Write Model）**:
- `backend/src/main/java/com/example/videowatchlog/domain/model/Title.java` - Series フィールド削除
- `backend/src/main/java/com/example/videowatchlog/domain/model/Series.java` - Episode フィールド削除
- `backend/src/main/java/com/example/videowatchlog/infrastructure/persistence/entity/TitleEntity.java`
- `backend/src/main/java/com/example/videowatchlog/infrastructure/persistence/entity/SeriesEntity.java`
- `backend/src/main/java/com/example/videowatchlog/infrastructure/persistence/TitleRepositoryImpl.java` - Series をロードしない
- `backend/src/main/java/com/example/videowatchlog/infrastructure/persistence/SeriesRepositoryImpl.java` - Episode をロードしない
- `backend/src/main/java/com/example/videowatchlog/application/usecase/GetAllTitlesUseCase.java` - Read Model 使用
- `backend/src/main/java/com/example/videowatchlog/application/usecase/GetTitleDetailUseCase.java` - Read Model 使用
- `backend/src/main/java/com/example/videowatchlog/application/usecase/CreateSeriesUseCase.java` - 簡潔化
- `backend/src/main/java/com/example/videowatchlog/application/usecase/CreateEpisodeUseCase.java` - 簡潔化

**テスト修正**:
- Domain層テスト: TitleTest, SeriesTest
- Repository層テスト: TitleRepositoryImplTest, SeriesRepositoryImplTest
- UseCase層テスト: 各ユースケーステスト
- Read Model テスト: TitleReadMapperTest, TitleReadServiceTest（新規）
- 統合テスト: 全APIエンドポイントテスト

---

## Notes

**本プランは `/speckit.plan` コマンドにより Phase 0-1 まで完了しました。**

現在の進捗:
- ✅ Phase 0（リサーチ）: 完了 - research.md 生成
- ✅ Phase 1（設計）: 完了 - data-model.md、contracts/、quickstart.md 生成
- 🔜 Phase 2（タスク・実装）: `/speckit.tasks` で tasks.md 生成、TDD に従い実装
- 🔜 Phase 3（アーキテクチャ改善）: CQRS + 独立集約パターン（別途計画）

**重要なリファレンス**:
- 仕様書: `specs/001-watch-list/spec.md`
- データモデル: `specs/001-watch-list/data-model.md`
- API 契約: `specs/001-watch-list/contracts/openapi.yaml`
- クイックスタート: `specs/001-watch-list/quickstart.md`
- リサーチ結果: `specs/001-watch-list/research.md`
- 憲法: `.specify/memory/constitution.md`
